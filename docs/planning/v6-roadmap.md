...
| 1.9 | Index **GiST composites** `btree_gist (tenant_id, shape)` — **35 colonnes géométriques sur 24 tables** du plan de données (mesuré au point 1.5 ; les autres sont au référentiel, sans `tenant_id`). **Insuffisant seul** : sous RLS, une condition non `LEAKPROOF` est évaluée après la politique et ne peut pas servir d'index. Marquer `geometry_overlaps` et `st_intersects` est une **décision de sécurité** à prendre explicitement. **Mesuré à l'échelle le 15 septembre** (200 fermes, 600 000 lignes) : la requête parcellaire passe par le GiST composite et ne lit que **80 lignes en 0,54 ms**, contre 2 000 lignes quand une des trois conditions manque. La deuxième condition a été trouvée là : une politique écrite en `OR` produit un `BitmapOr` qui rejette le prédicat spatial en filtre — elle tient donc en un seul `= ANY (readable_tenants())` | 35 colonnes |
...
| 1.21 | Retirer Apartment (9 fichiers) et le schéma d'agrégation. **Ce n'est plus une migration de production** (décision du 15 septembre 2026) : la V6 ne reprendra pas la base de la V5 en place. Les données d'un client arriveront par **dump de tenant, restauré par une méthode d'import**, une ferme à la fois — ce qui fait de `monoschema:migrate` non pas l'outil d'une bascule unique, mais le **chemin de restauration** permanent. Corollaire : pas de fenêtre d'indisponibilité à prévoir, et pas d'ordre de passage des fermes | — |
| 1.22 | ~~Chemin inter-tenants **explicite et audité**~~ | **fait, et sans rôle dédié ni bypass** — ce qui vaut mieux : `Ekylibre::Tenancy.across(ids, purpose:)` élargit la lecture *dans* la politique. `ekylibre.shared_tenants()` croise ce que le chemin demande avec ce que la table `tenant_shares` autorise : une ferme qui n'a pas consenti reste invisible même si on la demande (mesuré). L'écriture, elle, ne s'élargit jamais — un invariant d'audit échoue sur toute politique dont le `WITH CHECK` mentionnerait la fonction. L'appel est journalisé | le gain fonctionnel du lot |
