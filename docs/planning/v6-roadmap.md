...
| 1.16 | Déclarer les clés étrangères composites en **`foreign_key: %i[tenant_id …]`** — `query_constraints:` est refusé sur une association depuis Rails 8.1, il ne reste qu'une notion de modèle. S'y ajoute une chasse que le prototype a révélée : sous clé composite **`record.id` rend le couple**, si bien que tout `foo_id: bar.id` devient faux en silence ; c'est `id_value` qu'il faut | **1 413 associations** |
| 1.17 | Contexte runtime : `SET LOCAL app.tenant_id` en transaction, `Current.tenant`, **fail-closed**. Deux exigences mesurées au point 1.4 : **vider le cache de requêtes** à tout changement de contexte — il est indexé sur le seul texte SQL et resservira les lignes du tenant précédent, y compris hors contexte — et trancher le cas des lectures hors transaction, où `SET LOCAL` ne survit pas | — |
...
| 1.20 | Tests d'isolation **générés** : sans contexte → 0 ligne ; sous tenant A → jamais une ligne de B. Attention au cache de requêtes (cf. 1.17) : sans vidage entre deux contextes, ces tests se prouvent l'un l'autre | une paire par modèle du plan de données |
