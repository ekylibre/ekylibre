...
| A.4 | **BLOQUÉ — ne peut être terminé sans conception documentaire.** L'ADR-6.1 reposait sur « 14 `.jrxml` à migrer » ; la vraie mesure est le nombre de *natures* sans `.odt`, car `TemplateFileProvider` sert l'ODT en priorité et le Jasper en repli. **13 natures** sur 73 n'ont ni ODT ni Printer et ne peuvent donc être servies que par Jasper. Chacune exige un gabarit `.odt` (document LibreOffice — pas du code) en plus de sa classe `Printers::*`. Fait : suppression de 5 gabarits morts, sans effet sur la résolution des 73 natures. Détail : [audit §8](v6-dependency-audit.md) | 10 → **13 gabarits ODT à produire** + ~5 j·h de code |
