...
| A.4 | **7/13 natures migrées (option 2).** Printers + gabarits ODT provisoires générés par `bin/generate_odt_template.rb`, branche PDF câblée avant `respond_with` dans 6 contrôleurs. **La surface Jasper atteignable depuis l'interface est nulle.** Restent 6 natures sans bouton d'export : 2 mortes et 4 dans `HIDDEN_AGGREGATORS`. Retirer `rjb`/`beardley*` demande de les migrer ou de les sortir de la nomenclature — arbitrage produit. Les 7 gabarits sont à remettre en page dans LibreOffice. [Audit §8](v6-dependency-audit.md) | ~2 j·h restants + mise en page |
