...
| A.2 | **En cours — cible corrigée : pas AASM.** Le projet a déjà son remplaçant maison, le concern `Transitionable` (`app/models/concerns/transitionable.rb`, dont l'en-tête dit *« aims to replace the use of the state_machine gem »*), et 3 modèles sont déjà migrés (`fixed_asset`, `loan`, `reception`, 7 transitions écrites). Introduire AASM ajouterait un **troisième** mécanisme. Motif retenu : `include Transitionable` + `enumerize :state, predicates: true, i18n_scope:` + une classe `Transition` par événement dans `app/services/<modèle>/transitions/`. **Fait (7/14) : `payslip`, `tax_payment`, `journal_entry_item`, `task`, `issue`, `tax_declaration`, `purchase_order`** (+ `Purchase#state_label` détaché de la gem). Restent 7 modèles / 34 événements : `sale`, `journal_entry` (seul `before_transition`), `delivery`, `shipment`, `contract`, `sale_opportunity`, `activity_production` | 9 |
