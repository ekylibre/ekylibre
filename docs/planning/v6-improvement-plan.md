...
| A.2 | **En cours — cible corrigée : pas AASM.** Le projet a déjà son remplaçant maison, le concern `Transitionable` (`app/models/concerns/transitionable.rb`, dont l'en-tête dit *« aims to replace the use of the state_machine gem »*), et 3 modèles sont déjà migrés (`fixed_asset`, `loan`, `reception`, 7 transitions écrites). Introduire AASM ajouterait un **troisième** mécanisme. Motif retenu : `include Transitionable` + `enumerize :state, predicates: true, i18n_scope:` + une classe `Transition` par événement dans `app/services/<modèle>/transitions/`. **Fait (10/14)** : `payslip`, `tax_payment`, `journal_entry_item`, `task`, `issue`, `tax_declaration`, `purchase_order`, `contract`, `sale_opportunity`, `activity_production`. **Attention** : la gem est aussi introspectée hors des modèles — `Model.state_machine.events` dans 5 contrôleurs, `Model.state_machine.states` dans 4 vues d'index, `human_state_name` dans 6 colonnes de listes. Ces sites doivent être migrés **en même temps** que leur modèle (`StateBarBuilder`, lui, est déjà bimodal). Restent 4 modèles / 19 événements : `sale`, `journal_entry` (seul `before_transition`), `delivery`, `shipment` | 5 |
