...
| # | Action | Effort |
|---|---|---:|
| B.1 | Ruby 2.6 → 3.3. **Rails 5.2 démarre et tourne sous Ruby 2.7** (images `ruby2.7` et `ruby3.3` désormais publiques sur GHCR). L'exécution sous 2.7 a livré l'inventaire exhaustif que l'analyse statique ne pouvait pas donner — 2 ruptures bloquantes et 6 sites à mots-clés, tous corrigés. **Dev et CI sont passés sur `ruby2.7`** — image reconstruite, gems réinstallées, suite verte (fixtures 312/312). **Reste** : `Psych 4` sur 4 des 14 `serialize` (au palier B.3, le crochet n'existant pas en 5.2), la production, encore en 2.6, et le saut vers 3.3 | 10 → **2** |
...
>
> **Ce qui reste, et que le lot B ne doit pas porter** : sept valeurs par défaut désactivées, chacune documentée dans `config/application.rb` avec le travail qu'elle attend. Les deux plus lourdes sont `raise_on_assign_to_attr_readonly` — 325 tests, des écritures aujourd'hui perdues en silence dans les rappels comptables — et `raise_on_missing_required_finder_order_columns`, qui attend que les quatorze tables du lexique sans clé reçoivent la leur : c'est le lot C. S'ajoute une instabilité de la suite à traiter pour elle-même : un test d'achat aux montants inconciliables bascule six fois sur quatorze exécutions mesurées, ce qui a d'abord été imputé à tort à `Regexp.timeout` — le réglage l'aggrave (cinq sur cinq contre une sur cinq) sans l'expliquer. S'y ajoutent la sortie de `wice_grid` (qui retient `default_column_serializer`), celle de `turnout` (qui retient `rack` sous la 3, donc `sidekiq` sous la 8), et **un commit à pousser sur la branche `6.0` du greffon `baqio`**, sans quoi la CI rejouera le `TSort::Cyclic` du démarrage.
