...
export class DukeWidget {
    ...
    _startWebSpeech() {
        ...
        this._committedTranscript = this.textarea.value ? this.textarea.value.trimEnd() + ' ' : '';
    ...
    async _startServerRecording() {
        ...
        try {
            ...
            this._appendSystem(e?.name === 'NotAllowedError' ? I18N.micPermission : I18N.micError);
            return;
        }
    ...
    async _connect() {
        ...
        // browser-native path.
        if (this.micButton.hidden && this.sttServerEnabled && this.sttUrl && hasMediaRecorder()) {
            this.micButton.hidden = false;
        }
    ...
    _populateLlmSelector(authOk) {
        ...
        this.llmSelect.innerHTML = providers.map((p) => `<option value="${p}">${I18N.llmNames[p] || p}</option>`).join('');
    ...
    _renderDraft(msg) {
        ...
        const targets = (fields.targets || []).map((t) => this._escape(t.resolved_name || t.raw_name)).join(', ');
        const inputs = (fields.inputs || [])
            ...
            .join(', ');
        const tools = (fields.tools || []).map((t) => this._escape(t.resolved_name || t.raw_name)).join(', ');
        const doers = (fields.doers || []).map((d) => this._escape(d.resolved_name || d.raw_name)).join(', ');
        ...
        const ambiguities = (msg.ambiguities || [])
            .map((a, idx) => {
                ...
                const opts = options
                    ...
                    .join('');
                return `<li>${this._escape(a.question)}` + (opts ? `<div class="duke-draft__options">${opts}</div>` : '') + '</li>';
            })
            .join('');
    ...
    `;
        ...
        node.querySelectorAll('.duke-draft__multi-options').forEach((group) => {
            ...
            submitBtn.addEventListener('click', () => {
                ...
                group.querySelectorAll('input[type="checkbox"]:not(:disabled)').forEach((i) => (i.disabled = true));
