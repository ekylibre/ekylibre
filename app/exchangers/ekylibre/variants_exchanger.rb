    ...
    s.sheets.each do |sheet_name|
      ...
      2.upto(s.last_row) do |row|
        ...
        if r.variant_reference_name
          ...
          if r.purchase_unit_pretax_amount && catalog = Catalog.where(usage: :purchase).first
          ...
          if r.stock_unit_pretax_amount && catalog = Catalog.where(usage: :stock).first and variant.catalog_items.where(catalog_id: catalog.id).empty?
          ...
          if r.sale_unit_pretax_amount && catalog = Catalog.where(usage: :sale).first and variant.catalog_items.where(catalog_id: catalog.id).empty?
