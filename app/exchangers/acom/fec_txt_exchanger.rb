...
module Acom
  class FecTxtExchanger < ActiveExchanger::Base
    ...
    def check
      ...
      rows.each_with_index do |row, index|
        ...
        if row.account_number.blank? || row.printed_on.blank? || row.entry_item_name.blank?
          w.error "Errors on account (#{row.account_number}) or printed_on (#{row.printed_on.to_s}) or entry_item_name (#{row.entry_item_name}) in line : #{line_number} - #{valid}".red
          valid = false
        end
    ...
    def import
      ...
      journal = find_or_create_journal('ACOM', 'Import ACOM', 'various')
      ...
      rows.each_with_index do |row, index|
        ...
        number = row.printed_on.strftime("%Y%m%d") + row.journal_code + row.continuous_number.to_s
