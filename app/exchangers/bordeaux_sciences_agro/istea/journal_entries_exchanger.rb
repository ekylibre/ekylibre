        ...
        valid = true
          ...
          rows.each_with_index do |row, index|
            ...
            r = {
              ...
              journal_name: row[3].to_s,
              entry_name: row[4].to_s,
              account_number: row[6].to_s,
              entry_item_name: row[17].to_s,
              debit_amount: (row[18].blank? ? 0.0 : row[18].to_d),
              credit_amount: (row[19].blank? ? 0.0 : row[19].to_d),
              pretax_base: (row[21].blank? ? 0.0 : row[21].to_d)
            }.to_struct
            # w.check_point
