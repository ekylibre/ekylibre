module BordeauxSciencesAgro
  module ISTEA
    class GeneralLedgerExchanger < ActiveExchanger::Base
      def check
        blank_string = ''.freeze
      ...
      def import
        ...
        rows.collect! do |row|
          ...
          unless year
            raise 'Cannot find financial year for: ' + Date.parse(row[3]).inspect + ' (' + row[3].inspect + '). Expecting one of: ' + FinancialYear.all.pluck(:code).to_sentence
          end
          row << year.code
        end
        w.count = rows.size
