  ...
  class OutgoingDeliveriesExchanger < ActiveExchanger::Base
    def import
      ...
      rows.each do |row|
        ...
        # create variables
        campaign = Campaign.of(r.delivery_on.year)
        silo = Equipment.find_by(work_number: r.building_division_work_number)
        if r.cultivable_zone_work_number
          cultivable_zone = CultivableZone.find_by(work_number: r.cultivable_zone_work_number)
        end
        ps = ActivityProduction.where(cultivble_zone: cultivable_zone).of_campaign(campaign) if cultivable_zone
