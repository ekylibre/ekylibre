...
module Sage
  module Coala
    class FecTxtExchanger < ActiveExchanger::Base
      ...
      #  4 - E: CompteNum : "468600000000" # to truncate to 8
      ...
      def import
        ...
        # find or create a unique journal for SAGE
        journal = find_or_create_journal('SAGE', 'Import Koala', 'various')

        entries = {}
        rows.each_with_index do |row, index|
          line_number = index + 2

          raw_number = row.printed_on.strftime("%Y%m%d") + row.journal_code + row.continuous_number.to_s
          ...
          entries[number] ||= {
            ...
            journal: journal,
            number: number,
            continuous_number: row.continuous_number,
          ...
          if row.account_number.start_with?(client_account_radix, supplier_account_radix)
            if row.auxiliary_account_number && row.auxiliary_account_name
              account = find_or_create_account(row.auxiliary_account_number, row.auxiliary_account_name)
              find_or_create_entities(row.printed_on, account, row.auxiliary_account_number, row.auxiliary_account_name)
              w.info "third account & entity created: #{account.label.inspect.green}"
            else
              w.error "Supplier or Client account #{row.account_number} without auxiliary_account_number or auxiliary_account_name"
            end
          else
      ...
      private
        ...
        def create_account(acc_number, acc_number_normalized, acc_name)
          ...
          if acc_number.start_with?(client_account_radix, supplier_account_radix)
            aux_number = acc_number[client_account_radix.length..-1].upcase.gsub(/[!@%&"]/, '')
        ...
        # @param [Account] acc
        # @return [Entity]
        def find_or_create_entities(period_started_on, acc, provider_account_number, entity_name)
          entity = Maybe(find_entities_by_provider(provider_account_number))
            .recover { find_entities_by_account(acc) }
            .recover { create_entity(period_started_on, acc, provider_account_number, entity_name) }
            .or_raise

          if entity && (entity.first_met_at.nil? || (period_started_on && period_started_on < entity.first_met_at.to_date))
        ...
        # @param [String] provider_account_number
        # @return [Entity]
        def create_entity(period_started_on, account, provider_account_number, entity_name)
      ...
      protected
        ...
        # CAUTION we use build_deprecated_for_account_creation because 301/302 with 12 charaters won't be normalized and could be considered for a variant account
        # ex 30110000000 raise error because we could have 301100000000 in some case
        def account_normalizer
          @account_normalizer ||= Accountancy::AccountNumberNormalizer.build_deprecated_for_account_creation
        end
