  ...
  module ISeven
    class FecTxtExchanger < ActiveExchanger::Base
      ...
      NORMALIZATION_CONFIG = [
        ...
        { col: 11, name: :amount, type: :float },
        { col: 12, name: :sens, type: :float },
      ...
      def import
        ...
        journal = find_or_create_journal('SAGE', 'SAGE', 'various')

        entries = {}
        rows.each_with_index do |row, index|
          line_number = index + 2

          # case of debit values
          if row.sens == -1.0
      ...
      protected
        ...
        def parse_file(file)
          rows = ActiveExchanger::CsvReader.new(col_sep: nil).read(file)
