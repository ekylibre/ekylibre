    ...
    def import
      ...
      rows.each do |row|
        r = OpenStruct.new(
          ...
          at: (row[7].blank? ? nil : Date.civil(*row[7].to_s.split(%r{/}).reverse.map(&:to_i))),
          ...
          sampled_at: (row[179].blank? ? nil : Date.civil(*row[179].to_s.split(%r{/}).reverse.map(&:to_i)))
        )
