(($) ->
  ...
  setUpdateTriggersJournalEntryItems = ->
    ...
    links.each ->
      link = $(this)
      triggerSelector = link.data('complete-link-with-journal-entry-items-ids')
      $(triggerSelector).change ->
        url = link.attr('href')
        triggeredInput = $(this)
        id = triggeredInput.attr 'id'
  ...
  $(document).behave 'load', 'a[data-complete-link-with-journal-entry-items-ids]', setUpdateTriggersJournalEntryItems

  completeUrlWithJournalEntryItemsIds = ->
    links = $(this)
    links.each ->
      link = $(this)
      paramName = $(link).attr('data-complete-link-with-journal-entry-items-ids')
      checkboxes = $('.list td input[type="checkbox"]')
      ...
      $(checkboxes).change ->
        ...
        if url.indexOf('&nature=various') < 0 then url += "&nature=various"
  ...
  $(document).behave 'load', 'a.js-complete-links-with-journal-entry-items-ids', completeUrlWithJournalEntryItemsIds
