      ...
      $.get
        ...
        success: (data, status, request) ->
          for item, index in data.items
            ...
            itemLine.push("<span class='item-amount'><input name='intervention[receptions_attributes][0][items_attributes][#{-index}][pretax_amount]' value='#{item.pretax_amount}' type='hidden'></input>" + item.unit_pretax_amount * item.quantity +  "</span>")
