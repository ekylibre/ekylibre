  ...
  module Cobbles
    class StockInGroundCobblesController < Backend::Cobbles::BaseController
      def show
        ...
        @measure_symbol  = Onoma::Unit.find(@last_inspection.user_quantity_unit(@dimension)).symbol
        @yield_symbol    = Onoma::Unit.find(@last_inspection.user_per_area_unit(@dimension)).symbol
      end
    end
  end
end
