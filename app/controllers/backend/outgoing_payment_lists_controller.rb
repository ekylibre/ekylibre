    ...
    def new
      ...
      if params[:started_at].present? && params[:stopped_at].present? && params[:outgoing_payment_list] && params[:outgoing_payment_list][:mode_id]
        ...
        if @outgoing_payment_list.valid?
          ...
          @thirds = Entity.includes(:purchase_affairs).where(affairs: { closed: false, currency: mode.cash.currency }).where('affairs.updated_at BETWEEN ? AND ?', params[:started_at], params[:stopped_at]).where('affairs.credit - affairs.debit > 0').order('affairs.third_id, affairs.created_at')
        end
      else
