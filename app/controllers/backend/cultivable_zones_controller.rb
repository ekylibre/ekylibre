...
module Backend
  class CultivableZonesController < Backend::BaseController
    ...
    def show
      ...
      t3e @cultivable_zone
      respond_with(@cultivable_zone, methods: [:shape_svg, :cap_number, :human_shape_area],
                                  include: [
                                    { activity_productions: {
                                      methods: [:name, :implanted_at, :harvested_at],
                                      include: {
                                        interventions: {
                                          methods: [:started_at, :stopped_at, :status, :name, :duration, :human_working_zone_area, :human_actions_names, :human_input_quantity_names],
                                          include: {}
                                        }
                                      }
