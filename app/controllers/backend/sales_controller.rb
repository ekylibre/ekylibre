...
class Backend::SalesController < BackendController
  ...
  def show
    ...
    respond_with(@sale, :methods => [:taxes_amount, :affair_closed, :code_client], :include => {:address => {}, :supplier => {:include => :default_mail_address}, :credits => {}, :invoice_address => {}, :items => {:methods => [:taxes_amount],:include => [:product, :price]}}) do |format|
      format.html do
