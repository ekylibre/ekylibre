...
module Backend
  class BankStatementsController < Backend::BaseController
    ...
    def reconciliation
      reconciliate if params[:cash_id].nil?

      unless params[:cash_id].nil?
        bank_statements = BankStatement.where(cash_id: params[:cash_id])  
      end
    end

    def interval_reconciliation; end
    ...
    def reconciliate(bank_statement_id: nil)
      return unless @bank_statement = find_and_check(id: bank_statement_id)
      ...
      @period_start = @bank_statement.started_on - 20.days if @period_start.nil?
      @period_end   = @bank_statement.stopped_on + 20.days if @period_end.nil?
