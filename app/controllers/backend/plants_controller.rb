...
module Backend
  class PlantsController < Backend::MattersController
    ...
    def self.list_conditions
      ...
      code << " fy_stoped_on = (fy ? fy.stopped_on : Time.zone.today)\n"
      code << " fy_started_on = (fy ? fy.started_on : Time.zone.today)\n"
      code << " c[0] << ' AND #{Plant.table_name}.born_at::DATE BETWEEN ? AND ?'\n"
      code << " if params[:born_at].to_s == 'interval'\n"
      code << "   if params[:born_at_started_on].present? && params[:born_at_stopped_on].present?\n"
      code << "     c << params[:born_at_started_on]\n"
      code << "     c << params[:born_at_stopped_on]\n"
      code << "   elsif params[:born_at_started_on].present?\n"
      code << "     c << params[:born_at_started_on]\n"
      code << "     c << fy_stoped_on\n"
      code << "   elsif params[:born_at_stopped_on].present?\n"
      code << "     c << fy_started_on\n"
      code << "     c << params[:born_at_stopped_on]\n"
      code << "   end\n"
      code << " else\n"
      ...
      code << "   if params[:dead_at_started_on].present? && params[:dead_at_stopped_on].present?\n"
      code << "     c << params[:dead_at_started_on]\n"
      code << "     c << params[:dead_at_stopped_on]\n"
      code << "   elsif params[:dead_at_started_on].present?\n"
      code << "     c << params[:dead_at_started_on]\n"
      code << "     c << fy_stoped_on\n"
      code << "   elsif params[:dead_at_stopped_on].present?\n"
      code << "     c << fy_started_on\n"
      code << "     c << params[:dead_at_stopped_on]\n"
      code << "   end\n"
      code << " else\n"
