    ...
    module Reconciliation
      ...
      class GapsController < Backend::BaseController
        ...
        def letter_and_redirect(bank_items, entry_items)
          ...
          redirect_to backend_bank_statements_reconciliation_items_path(@bank_statement, scroll_to: bank_items.order(transfered_on: :asc).first.id) unless @bank_statement.nil?

          redirect_to reconciliate_bank_statements_backend_bank_statements_reconciliation_items_path(cash_id: @cash.id, scroll_to: bank_items.order(transfered_on: :asc).first.id, period_start: @period_start, period_end: @period_end) unless @bank_statements.nil?
        end
