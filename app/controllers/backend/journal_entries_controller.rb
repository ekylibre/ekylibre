      ...
      if params[:duplicate_of]
        ...
        @journal_entry.printed_on = params[:printed_on] || Time.zone.today
          ...
          if params[:journals_entries_items_ids]
            ...
            sum_credit = 0
            sum_debit = 0
            journal_entry_items.each { |jei| sum_credit += jei.credit }
            journal_entry_items.each { |jei| sum_debit += jei.debit }
            account = journal_entry_items.first.account
            if sum_credit > sum_debit
              balanced_credit = 0
              balanced_debit = sum_credit - sum_debit
            elsif sum_credit < sum_debit
              balanced_debit = 0
              balanced_credit = sum_debit - sum_credit             
            end
            # Create a line which will the one used to balance the credit and debit sum amounts of lines selected in "account/:id/mark" view 
            @journal_entry.items.new(account: account, real_debit: balanced_debit, real_credit: balanced_credit)
            # Create a line to balance the previous line
            @journal_entry.items.new(real_debit: balanced_credit, real_credit: balanced_debit)
          end
      end
