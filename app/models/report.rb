...
class Report < ActiveRecord::Base
 ...
 # this function saves the pdf document genererated by the function render_report in the table reports in the database
  def self.register(template_md5,key,company_id,binary,title)  
    ...
    unless Report.exists?(["template_md5 = ? AND key = ?", template_md5, key])
      report=Report.create!(:key=>key,:template_md5=>template_md5,:sha256=>binary_digest, :original_name=>title, :printed_at=>Time.now,:company_id=>company_id)
