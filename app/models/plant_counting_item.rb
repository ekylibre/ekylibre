...
class PlantCountingItem < Ekylibre::Record::Base
  ...
  def update_average_value
    items = self.plant_counting.items
    average_value = item.any? ? items.average(:value) : nil
    self.plant_counting.update_column(:average_value, average_value)
  end

  def siblings
    self.plant_counting.items
  end
