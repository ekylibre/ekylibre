module Indicateable
  ...
  def substract_to_indicator_data(indicator, value, options = {})
    operate_on_indicator_data(indicator, value, options.merge(operation: :substract))
  end

  # Substract a value to a list of indicator data
  def add_to_indicator_data(indicator, value, options = {})
    operate_on_indicator_data(indicator, value, options.merge(operation: :add))
  end


  def operate_on_indicator_data(indicator, value, options = {})
    unless indicator.is_a?(Nomen::Item) or indicator = Nomen::Indicators[indicator]
      raise ArgumentError, "Unknown indicator #{indicator.inspect}. Expecting one of them: #{Nomen::Indicators.all.sort.to_sentence}."
    end
    data = self.indicator_data.where(indicator_name: indicator.name)
    operation = options.delete(:operation)
    if options[:before]
      data = data.where("measured_at <= ?", options[:before])
    end
    if options[:after]
      data = data.where("measured_at >= ?", options[:after])
    end
    if operation == :add
