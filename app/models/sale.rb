...
class Sale < Ekylibre::Record::Base
  ...
  bookkeep do |b|
    b.journal_entry(self.nature.journal, reference_number: number, printed_on: invoiced_on, if: (with_accounting && invoice? && items.any?)) do |entry|
      ...
      items.each do |item|
        entry.add_credit(label, (item.account || item.variant.product_account).id, item.pretax_amount, activity_budget: item.activity_budget, team: item.team, as: :item_product, resource: item, variant: item.variant, accounting_label: item.accounting_label ? "#{item.accounting_label} (#{initial_number})" : nil)
        tax = item.tax
        entry.add_credit(label, tax.collect_account_id, item.taxes_amount, tax: tax, pretax_amount: item.pretax_amount, as: :item_tax, resource: item, variant: item.variant, accounting_label: item.accounting_label ? "#{item.accounting_label} (#{initial_number})" : nil)
      end
    end
