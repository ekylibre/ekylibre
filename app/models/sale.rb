...
class Sale < Ekylibre::Record::Base
  ...
  bookkeep do |b|
    b.journal_entry(self.nature.journal, printed_on: invoiced_on, if: (with_accounting && invoice? && items.any?)) do |entry|
      ...
      items.each do |item|
        entry.add_credit(label, (item.account || item.variant.product_account).id, item.pretax_amount, activity_budget: item.activity_budget, team: item.team, as: :item_product, resource: item, variant_id: item.variant_id)
        tax = item.tax
        entry.add_credit(label, tax.collect_account_id, item.taxes_amount, tax: tax, pretax_amount: item.pretax_amount, as: :item_tax, resource: item, variant_id: item.variant_id)
      end
    end
    ...
    b.journal_entry(journal, printed_on: invoiced_on, as: :undelivered_invoice, if: (with_accounting && invoice?)) do |entry|
      parcels.each do |parcel|
        ...
        undelivered_items.each do |undelivered_item|
          ...
          entry.add_credit(label, undelivered_item.account.id, undelivered_item.real_balance, resource: undelivered_item, as: :item_product, variant_id: undelivered_item.variant_id)
        end
      end
    end
    ...
    b.journal_entry(journal, printed_on: invoiced_on, as: :quantity_gap_on_invoice, if: (with_accounting && invoice? && items.any?)) do |entry|
      ...
      items.each do |item|
        ...
        entry.add_credit(label, item.variant.stock_account_id, gap_value, resource: item, as: :stock, variant_id: item.variant_id)
        entry.add_debit(label, item.variant.stock_movement_account_id, gap_value, resource: item, as: :stock_movement, variant_id: item.variant_id)
      end
    end
  end
