...
class FinancialYear < Ekylibre::Record::Base
  ...
  enumerize :state, in: %i[opened closed locked close_in_preparation], default: :opened, predicates: true
  ...
  scope :closables_or_lockables, -> { where(state: 'opened').where('? BETWEEN started_on AND stopped_on', Time.zone.now).reorder(:started_on) }
