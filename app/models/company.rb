...
class Company < ActiveRecord::Base
  ...
  def after_create
    ...
    self.shelves.create(:name=>lc(:default_shelf_name))
...
#    self.widgets.create!(:name=>'test', :location=>Location.find_by_name('side'), :nature=>'content')
   # self.currencies.create!(:name=>'euro', :code=>'E', :format=>'eu', :rate=>6.53)
    ...
    testf = self.financialyears.create!(:code=>'codetest', :started_on=>Date.new(2009,01,01) , :stopped_on=>Date.new(2009,12,31) , :company_id=>self.id)
    tf = self.financialyears.create!(:code=>'coffdetest', :started_on=>Date.new(2007,01,01) , :stopped_on=>Date.new(2007,12,31) , :company_id=>self.id)
    t3 = self.financialyears.create!(:code=>'codetecvdfst', :started_on=>Date.new(2008,01,01) , :stopped_on=>Date.new(2008,12,31) , :company_id=>self.id)
