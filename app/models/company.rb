...
#    self.widgets.create!(:name=>'test', :location=>Location.find_by_name('side'), :nature=>'content')
   # self.currencies.create!(:name=>'euro', :code=>'E', :format=>'eu', :rate=>6.53)
    ...
    testf = self.financialyears.create!(:code=>'codetest', :started_on=>Date.new(2009,01,01) , :stopped_on=>Date.new(2009,12,31) ,:written_on=>Date.new(2007,05,05) , :company_id=>self.id)
