...
class Company < ActiveRecord::Base
  ...
  def after_create
    ...
    test = self.accounts.create!(:number=>42, :alpha=>7, :name=>'testt', :label=>'jddk', :usable=>true, :groupable=>true, :keep_entries=>true, :parent_id=>1 )
    ...
    #self.account_balances.create!(:account_id=>test.id , :financialyear_id=>testf.id , :global_credit=>15000 , :company_id=>self.id  )
