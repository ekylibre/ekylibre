...
#    end
 ...
 def to_accountancy
   ...
   unless self.collected_amount.zero?
     @entry = self.company.journal_entries.create!({:record_id => @record.id, :currency_id => @journal_od.currency_id, :account_id =>self.company.parameter('accountancy.accounts.collected_taxes').value.to_s, :name => '', :currency_debit => self.collected_amount})
   end
   
   unless self.acquisition_amount.zero?
     @entry = self.company.journal_entries.create!({:record_id => @record.id, :currency_id => @journal_od.currency_id, :account_id =>self.company.parameter('accountancy.accounts.acquisitions_taxes').value.to_s, :name => '', :currency_debit => self.acquisition_amount})
   end
   
   unless self.assimilated_taxes_amount.zero?
     @entry = self.company.journal_entries.create!({:record_id => @record.id, :currency_id => @journal_od.currency_id, :account_id =>self.company.parameter('accountancy.accounts.assimilated_taxes').value.to_s, :name => '', :currency_debit => self.assimilated_taxes_amount})
   end
   
   unless self.paid_amount.zero?
     @entry = self.company.journal_entries.create!({:record_id => @record.id, :currency_id => @journal_od.currency_id, :account_id =>self.company.parameter('accountancy.accounts.paid_taxes').value.to_s, :name => '', :currency_credit => self.paid_amount})
   end
   
   unless self.balance_amount.zero?
     @entry = self.company.journal_entries.create!({:record_id => @record.id, :currency_id => @journal_od.currency_id, :account_id => self.company.parameter('accountancy.accounts.balance_taxes').value.to_s, :name => '', :currency_credit => self.balance_amount})
   end
