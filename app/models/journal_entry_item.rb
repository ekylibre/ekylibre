...
class JournalEntryItem < Ekylibre::Record::Base
  ...
  def product_item_to_tax_label
    vat_journal_entry_item = self.class.find_by(resource_id: self.resource_id, resource_prism: 'item_tax') #where.not(id: self.id).
    if vat_journal_entry_item && vat_journal_entry_item.id != self.id
