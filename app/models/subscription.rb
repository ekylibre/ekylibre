...
class Subscription < ApplicationRecord
  ...
  validates :current_period_started_at, :current_period_stopped_at, :trial_started_at, :trial_stopped_at, timeliness: { on_or_after: -> { Time.new(1, 1, 1).in_time_zone }, on_or_before: -> { Time.zone.now + 100.years } }, allow_blank: true
  validates :description, length: { maximum: 500_000 }, allow_blank: true
  validates :number, :status, length: { maximum: 500 }, allow_blank: true
  ...
  validates :stopped_on, timeliness: { on_or_after: ->(subscription) { subscription.started_on || Time.new(1, 1, 1).in_time_zone }, on_or_before: -> { Time.zone.today + 100.years }, type: :date }, allow_blank: true
