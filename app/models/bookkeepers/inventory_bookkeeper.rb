  ...
  def call
    ...
    journal_entry(journal, printed_on: achieved_at.to_date, if: (financial_year && reflected?)) do |entry|
      ...
      item_variants.each do |variant|
        ...
        current_stock_amount = s.journal_entry_items_calculate(:balance, fy_started_at, achieved_at)
        raise NegativeStockValueError.new(s, fy_started_at, achieved_at, current_stock_amount) if current_stock_amount < 0
        ...
        # bookkeep step 2
        # if current_stock_amount.zero?
        stock_movement_amount = inventoried_stock_amount - current_stock_amount
