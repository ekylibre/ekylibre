...
class Activity < Ekylibre::Record::Base
  ...
  def color
    ...
    color_cycle = { gold: 'FFD700', golden_rod: 'DAA520', yellow: 'FFFF00', orange: 'FF8000',
                    red: 'FF0000', green: '80FF00', spring_green: '00FF7F', dark_turquoise: '00FFFF',
                    blue: '0000FF', purple: 'BF00FF', gray: 'A4A4A4', dark_magenta: '8B008B', violet: 'EE82EE', teal: '008080'}
    if activity_family
      # level 1
      if cultivation_variety <= :plant
        # level 2 - family
        if activity_family.parent <= :arboriculture
          ...
          # level 3 - category - BEETS = VIOLET
          elsif activity_family.parent <= :beet_crops 
            rgb_value << color_cycle[:violet]
          # level 3 - category - FODDER = SPRING GREEN
          elsif activity_family.parent <= :fallow_land || activity_family.parent <= :fodder_crops || activity_family.parent <= :meadow
            rgb_value << color_cycle[:spring_green]
