...
class ProductMovement < Ekylibre::Record::Base
  ...
  validates :stopped_at, timeliness: { allow_blank: true, on_or_after: :started_at }, if: ->(product_movement) { product_movement.stopped_at && product_movement.started_at }
  validates :delta, :population, numericality: { allow_nil: true }
  validates :delta, :population, :product, :started_at, presence: true
  # ]VALIDATORS]
