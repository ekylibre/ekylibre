...
module SaleInvoices
  class ReceiptsExtractor
    ...
    def create_zip(*financial_years)
      Dir.mktmpdir do |tmpdir|
        ...
        financial_years.each do |fy|
          ...
          fy_path.mkdir
          sale_invoices = Sale.invoiced_between(fy.started_on, fy.stopped_on.end_of_day).reorder(:invoiced_at)
          pis_by_month = sale_invoices.group_by { |pi| [pi.invoiced_at.strftime('%m'), pi.invoiced_at.strftime('%Y')] }
    ...
    def make_first_page(invoices, dir)
      ...
      # heading
      arr_invoices << [
          Sale.human_attribute_name(:invoiced_at),
          Sale.human_attribute_name(:number),
          Sale.human_attribute_name(:client),
          Sale.human_attribute_name(:pretax_amount),
          :tax_amount.tl
        ]
