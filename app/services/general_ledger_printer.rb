...
class GeneralLedgerPrinter
  ...
  def run(params = {})
    ...
    params[:started_on] ||= '2016-01-01'
    params[:stopped_on] ||= '2016-12-31'
    ...
    general_ledger_dataset = Account.ledger(params[:started_on], params[:stopped_on]) if params
    ...
    # TODO: add a generic template system path
      report = ODFReport::Report.new(Rails.root.join('config', 'locales', 'fra', 'reporting', 'general_ledger.odt')) do |r|
        ...
        r.add_field 'STARTED_ON', params[:started_on].to_date.strftime('%d/%m/%Y')
        r.add_field 'STOPPED_ON', params[:stopped_on].to_date.strftime('%d/%m/%Y')
