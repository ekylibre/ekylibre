class SalesInvoicePrinter
  ...
  def initialize(sale)
    @template_path   = find_open_document_template('sales_invoice')
    @sale            = sale
    @document_nature = Nomen::DocumentNature.find(document_nature)
  end
  ...
  def key
    "#{Nomen::DocumentNature.find(document_nature).name}-#{Time.zone.now.l(format: '%Y-%m-%d-%H:%M:%S')}"
  end
end
