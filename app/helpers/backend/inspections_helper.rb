    ...
    def data_to_details_hash(inspection)
      ...
      inspection.scales.order(:id).each do |scale|
        ...
        [:items_count, :net_mass].each do |dimension|
          ...
          statable[dimension][:total] = { title: :total.tl, unicity: dimension }
          statable[dimension][:yield] = { title: Inspection.human_attribute_name(:gross_yield_value), unicity: dimension }
          statable[dimension][:market] = { title: InspectionCalibration.human_attribute_name(:marketable), unicity: dimension }
        ...
        inspection.calibrations.of_scale(scale).reorder(:id).each do |calibration|
          [:items_count, :net_mass].each do |dimension|
            columns[quantity[dimension]][:body] << decimal_cell(calibration.quantity_in_unit(dimension).round(0).l(precision: 0))
            columns[statable[dimension][:total]][:body] << decimal_cell(calibration.projected_total(dimension).round(0).l(precision: 0))
            columns[statable[dimension][:yield]][:body] << decimal_cell(calibration.quantity_yield(dimension).round(2).l(precision: 2))
            columns[statable[dimension][:market]][:body] << decimal_cell(calibration.marketable? ? calibration.marketable_quantity(dimension).round(0).l(precision: 0) : nil)
          end

          columns[name][:body] << { tag: :td, content: calibration.nature_name }
          columns[grading_sizes[:min]][:body] << decimal_cell(calibration.extremum_size(:min).round(0).l(precision: 0))
          columns[grading_sizes[:max]][:body] << decimal_cell(calibration.extremum_size(:max).round(0).l(precision: 0))
        end
        #########

        ### TOTALS
        [:items_count, :net_mass].each do |dimension|
          columns[quantity[dimension]][:total] << total_decimal_cell(inspection.quantity(dimension, scale).round(0).l(precision: 0))
          columns[statable[dimension][:total]][:total] << total_decimal_cell(inspection.projected_total(dimension, scale).round(0).l(precision: 0))
          columns[statable[dimension][:yield]][:total] << total_decimal_cell(inspection.quantity_yield(dimension).round(2).l(precision: 2))
          columns[statable[dimension][:market]][:total] << total_decimal_cell(inspection.marketable_quantity(dimension).round(2).l(precision: 0))
        end
      ...
      if inspection.points.any?
        ...
        [:items_count, :net_mass].each do |dimension|
          ...
          statable[dimension][:total]   = { title: :total.tl, unicity: dimension }
          statable[dimension][:yield]   = { title: Inspection.human_attribute_name(:gross_yield_value), unicity: dimension }
          statable[dimension][:market]  = { title: InspectionPoint.human_attribute_name(:marketable), unicity: dimension }
        ...
        inspection.points.joins(:nature).order('category, name').each do |point|
          [:items_count, :net_mass].each do |dimension|
            columns[quantity[dimension]][:body] << decimal_cell(point.quantity_in_unit(dimension).round(0).l(precision: 0))
            columns[statable[dimension][:total]][:body] << decimal_cell(point.projected_total(dimension).round(0).l(precision: 0))
          ...
          columns[grading_sizes[:min]][:body] << decimal_cell(point.extremum_size(:min).round(0).l(precision: 0))
          columns[grading_sizes[:max]][:body] << decimal_cell(point.extremum_size(:max).round(0).l(precision: 0))
        end
        #########
