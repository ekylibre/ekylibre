  ...
  class SalesExchanger < ActiveExchanger::Base
    ...
    def import
      ...
      rows.each_row_streaming(offset: 1, pad_cells: true, max_rows: rows.count - 2) do |row|
        ...
        sale_item_tax = find_or_create_tax(r.origin_vat_id, Preference[:country].to_sym)
        unless sale_item_tax
          raise "Cannot determine VAT (id : #{r.origin_vat_id}) for sale item: #{r.invoice_line_origin_reference_number} in invoice #{r.invoice_origin_reference_number}"
        end
        ...
        sale_item = nil
        if r.unit_pretax_amount && r.quantity
          # item
          ...
          if variant.has_indicator? :net_mass
            ...
            if unitary_net_mass_value.nonzero?
              sale_item.quantity = r.quantity.in(PALLET_QUANTITY_UNIT_NAME).convert(unitary_net_mass_unit) / unitary_net_mass_value
            end
          else
            sale_item.quantity = r.quantity
          end
          unless r.discount_percentage.blank?
            sale_item.reduction_percentage = r.discount_percentage
          end
          sale_item.tax = sale_item_tax
          sale_item.unit_pretax_amount = nil # r.unit_pretax_amount
          sale_item.unit_amount = nil
          sale_item.pretax_amount = r.pretax_amount
        ...
        w.info "END - LINE REFERENCE: #{r.invoice_origin_reference_number} - #{r.invoice_line_origin_reference_number} : sale_id(#{sale.id if sale}) / sale_item_id(#{sale_item.id if sale_item})".red
