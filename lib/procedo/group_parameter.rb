...
module Procedo
  ...
  class GroupParameter < Procedo::Parameter
    ...
    def group_parameters(recursively = false)
      parameters(recursively).select { |i| i.is_a?(Procedo::GroupParameter) }
    end

    def product_parameters(recursively = false)
      parameters(recursively).select { |i| i.is_a?(Procedo::ProductParameter) }
    end
    ...
    def find!(name)
      parameter = find(name)
      fail "Cannot find parameter: #{name.inspect}" unless parameter
      parameter
    end
    ...
    def each_product_parameter(recursively = false, &_block)
      each_parameter(recursively) do |parameter|
        yield(parameter) if parameter.is_a?(Procedo::ProductParameter)
      end
    end
    ...
    def each_group_parameter(recursively = false, &_block)
      each_parameter(recursively) do |parameter|
        yield(parameter) if parameter.is_a?(Procedo::GroupParameter)
      end
    end

    def add_product_parameter(name, type, options = {})
      options[:group] = self
      parameter = Procedo::ProductParameter.new(@procedure, name, type, options)
      @parameters[parameter.name] = parameter
    end

    def add_group_parameter(name, options = {})
      options[:group] = self
      parameter = Procedo::GroupParameter.new(@procedure, name, options)
      @parameters[parameter.name] = parameter
    end
