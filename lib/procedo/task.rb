  ...
  class Task
    ...
    def initialize(operation, element, default_id = nil)
      ...
      if element.has_attribute?('do')
        ...
        fail Procedo::Errors::MissingAttribute, "Attribute 'do' is mandatory"
      end
      @action = nil
    ...
    def need_parameters?
      wanted_indicators.any?
    end

    def wanted_indicators
      parameters.values.select do |parameter|
        parameter.is_a?(Procedo::Indicator) && !parameter.value?
      end
    end
