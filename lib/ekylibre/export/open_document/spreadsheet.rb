    ...
    module OpenDocument
      class Spreadsheet
        ...
        def initialize
          ...
          zile.puts(File.open(File.join(File.expand_path(File.dirname(__FILE__)), 'spreadsheet', 'styles.xml'), 'rb').read)
          ...
          zile.puts("<office:automatic-styles>"+
                    ...
                    "<style:style style:name=\"ro2\" style:family=\"table-row\"><style:table-row-properties style:row-height=\"#{height}cm\" fo:break-before=\"page\" style:use-optimal-row-height=\"false\"/></style:style>"+
        ...
        def self.generate(output, options={}, &block)
          document = self.new
          yield document
          return document.generate(output, options)
        end
