            ...
            for support in production.supports
              if support.active?
                ...
                if area = land_parcel.shape_area
                  ...
                  Ekylibre::FirstRun::Booker.intervene(:atomic_organic_fertilizing, year - 1, 9, 1, 0.96 * coeff, support: support) do |i|
                  ...
                  Ekylibre::FirstRun::Booker.intervene(:atomic_plowing, year - 1, 9, 15, 9.78 * coeff, support: support, parameters: {readings: {"base-atomic_plowing-0-500-1" => "plowed"}}) do |i|
                    i.add_cast(reference_name: 'driver',  actor: i.find(Worker))
                  ...
                  int = Ekylibre::FirstRun::Booker.intervene(:atomic_sowing, year - 1, 10, 15, 6.92 * coeff, range: 15, support: support, parameters: {readings: {"base-atomic_sowing-0-750-2" => 2000000 + rand(250000)}}) do |i|
                  ...
                  Ekylibre::FirstRun::Booker.intervene(:atomic_mineral_fertilizing, year, 3, 1, 0.96 * coeff, support: support) do |i|
                  ...
                  if w.count.modulo(3).zero? # AND NOT prairie
                    ...
                    Ekylibre::FirstRun::Booker.intervene(:atomic_spraying_on_cultivation, year, 4, 1, 1.07 * coeff, support: support) do |i|
            ...
            for support in production.supports
              if support.active?
                ...
                if area = land_parcel.shape_area
                  ...
                  Ekylibre::FirstRun::Booker.intervene(:atomic_plowing, year, 4, 15, 9.78 * coeff, support: support, parameters: {readings: {"base-atomic_plowing-0-500-1" => "plowed"}}) do |i|
                    i.add_cast(reference_name: 'driver',  actor: i.find(Worker))
                    i.add_cast(reference_name: 'tractor', actor: i.find(Product, can: "tow(plower)"))
                    i.add_cast(reference_name: 'plow',    actor: i.find(Product, can: "plow"))
                    i.add_cast(reference_name: 'land_parcel', actor: land_parcel)
                  ...
                  int = Ekylibre::FirstRun::Booker.intervene(:atomic_all_in_one_sowing, year, 5, 2, 6.92 * coeff, range: 15, support: support, parameters: {readings: {"base-atomic_all_in_one_sowing-0-1200-2" => 80000 + rand(10000)}}) do |i|
                    i.add_cast(reference_name: 'seeds',        actor: i.find(Product, variety: :seed, derivative_of: variety, can: "grow"))
                    ...
                    i.add_cast(reference_name: 'molluscicide',   actor: i.find(Product, variety: :preparation, can: "kill(mollusca)"))
                    i.add_cast(reference_name: 'molluscicide_to_input', population: (rand(0.2) + 1) * coeff)
                    i.add_cast(reference_name: 'sower',        actor: i.find(Equipment, can: "spread(preparation), sow, spray"))
                    i.add_cast(reference_name: 'driver',       actor: i.find(Worker))
                    i.add_cast(reference_name: 'tractor',      actor: i.find(Product, can: "tow(sower)"))
                    i.add_cast(reference_name: 'land_parcel',  actor: land_parcel)
                    i.add_cast(reference_name: 'cultivation',  variant: ProductNatureVariant.find_or_import!(variety).first, population: (area.to_s.to_f / 10000.0), shape: land_parcel.shape)
                  ...
                  Ekylibre::FirstRun::Booker.intervene(:atomic_mineral_fertilizing, year, 5, 25, 0.96 * coeff, support: support) do |i|
                    i.add_cast(reference_name: 'fertilizer',  actor: i.find(Product, variety: :preparation, can: "fertilize"))
                    i.add_cast(reference_name: 'fertilizer_to_spread', population: 0.4 + coeff * rand(0.6))
                    i.add_cast(reference_name: 'spreader',    actor: i.find(Product, can: "spread(preparation)"))
                    i.add_cast(reference_name: 'driver',      actor: i.find(Worker))
                  ...
                  if w.count.modulo(3).zero?
                    # Treatment herbicide 01-04 30-04
                    Ekylibre::FirstRun::Booker.intervene(:atomic_spraying_on_cultivation, year, 5, 27, 1.07 * coeff, support: support) do |i|
                  ...
                  Ekylibre::FirstRun::Booker.intervene(:atomic_hoeing, year, 6, 20, 3 * coeff, support: support, parameters: {readings: {"base-atomic_hoeing-0-500-1" => "covered"}}) do |i|
            ...
            if support.active? and support.irrigated?
              ...
              if area = land_parcel.shape_area
                ...
                if sowing_intervention = support.interventions.of_nature(:sowing).reorder(:started_at).last
                  ...
                  Ekylibre::FirstRun::Booker.intervene(:atomic_watering, year, 7, 15, 0.96 * coeff, support: support) do |i|
            ...
            for support in production.supports
              if support.active?
                ...
                if area = land_parcel.shape_area
                  ...
                  bob = nil
                  sowing = support.interventions.where(reference_name: "atomic_sowing").where("started_at < ?", Date.civil(year, 6, 6)).order("stopped_at DESC").first
                  if cultivation = sowing.casts.find_by(reference_name: 'cultivation').actor rescue nil
                    int = Ekylibre::FirstRun::Booker.intervene(:atomic_plant_mowing, year, 6, 6, 2.8 * coeff, support: support) do |i|
                      bob = i.find(Worker)
                    ...
                    Ekylibre::FirstRun::Booker.intervene(:atomic_straw_bunching, year, 6, 20, 3.13 * coeff, support: support) do |i|
            ...
            for support in production.supports
              if support.active?
                ...
                if area = land_parcel.shape_area
                  ...
                  sowing = support.interventions.where(reference_name: "atomic_sowing").where("started_at < ?", Date.civil(year, 7, 1)).order("stopped_at DESC").first
                  if cultivation = sowing.casts.find_by(reference_name: 'cultivation').actor rescue nil
                    Ekylibre::FirstRun::Booker.intervene(:atomic_grains_harvest, year, 7, 1, 3.13 * coeff, support: support) do |i|
