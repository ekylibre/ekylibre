...
Ekylibre::FirstRun.add_loader :analyses do |first_run|
  ...
  file = first_run.path("agro_systemes", "analyses_sol.csv")
  if file.exist?
    File.open(file, "rb") do |f|
      Import.launch!(:agro_systemes_soil_analyses, f)
    end
  end
    ...
    first_run.count :milk_analyses_import do |w|
      ...
      CSV.foreach(file, :encoding => "CP1252", :col_sep => "\t", :headers => true) do |row|
        r = OpenStruct.new(:year => row[0],
                           ...
                           :buty => (row[8].blank? ? 0 : row[8].gsub(",",".").to_i).in_unity_per_liter,
                           :cryo => (row[9].blank? ? 0.00 : row[9].gsub(",",".").to_d).in_celsius,
                           :lipo => (row[10].blank? ? 0.00 : row[10].gsub(",",".").to_d).in_thousand_per_hectogram,
