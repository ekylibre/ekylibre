...
Exchanges.add_importer :ekylibre_variants do |file, w|
                ...
                unless indicator = (unit.blank? ? :population : r.price_unity.second)
                  ...
                  if indics.count > 1
                    if indics.include?(default_indicators[dimension].to_s)
                      ...
                      raise Exchanges::NotWellFormedFileError, "Ambiguity on unit #{unit.inspect} for variant #{variant.name.inspect} between #{indics.to_sentence(locale: :eng)}. Cannot known what is wanted, insert indicator name after unit like: '#{unit} (#{indics.first})'."
                    end
                  elsif indics.empty?
                    if unit == "hour"
                      indicator = "working_duration"
                    else
                      raise Exchanges::NotWellFormedFileError, "Unit #{unit.inspect} is invalid for variant #{variant.name.inspect}. No indicator can be used with this unit."
                    end
                  else
