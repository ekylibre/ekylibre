Exchanges.add_importer :la_graine_informatique_vinifera_entities do |file, w|
  ...
  rows.each do |row|
    ...
    unless r.postal_code_city.blank? or person.mails.where("mail_line_6 ILIKE ?",  r.postal_code_city).where(mail_country: r.country).any?
