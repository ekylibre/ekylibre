...
module SVF
  ...
  class Cell
    ...
    def parse_value(line = 'line', start=nil)
      ...
      if self.type == :boolean
        ...
        value = "(#{line}[#{start}..#{start+size-1}]+'.'+#{line}[#{start+size+1}..#{start+self.length-1}]).to_f"
  ...
  class Formater
    ...
    def parse_code(siblings=[], options={})
      ...
      if parents.size > 0
        code << "if ["+all_parents.collect{|s| @lines[s.line].class_name(@name)}.sort.join(', ')+"].include?(line.class)\n"
