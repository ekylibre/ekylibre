Exchanges.add_importer(:lilco_milk_analyses) do |file, w|
  ...
  rows.each do |row|
    r = OpenStruct.new(:year =>  row[0],
                       ...
                       :cryo =>   (row[9].blank? ? 0 : row[9].gsub(",", ".").to_d).in_celsius,
                       :lipo =>  (row[10].blank? ? 0 : row[10].gsub(",", ".").to_d).in_thousand_per_hectogram,
