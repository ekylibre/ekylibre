  ...
  task :migration => :environment do
    class Default < ActiveRecord::Migration
      def self.up
      	...
      	company = Company.create!(:name=>'NaC', :code=>'NAC')
      	role = Role.create!(:name=>'Default', :default=>true, :company_id=>company.id)
      	User.create!(:name=>'admin', :first_name=>'Toto', :last_name=>'ADMIN', :password=>'4dm|n', :password_confirmation=>'4dm|n', :company_id=>company.id, :language_id=>language.id, :role_id=>role.id)
