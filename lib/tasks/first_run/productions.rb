...
load_data :productions do |loader|
  ...
  file = loader.path("alamano", "activities", "activity_families.csv")
  if file.exist?
  ...
  file = loader.path("alamano", "activities", "activity_natures.csv")
  if file.exist?
    CSV.foreach(file, headers: true) do |row|
  ...
  file = loader.path("alamano", "activities", "activities.csv")
  if file.exist?
          ...
          if product_nature_variant_sup and !product_support.nil?
            ...
            if product_support.is_a?(CultivableZone)
              ...
              if !r.provisional_nitrogen_input.nil?
                support.read!(:nitrogen_area_density, r.provisional_nitrogen_input.in_kilogram_per_hectare)
              end
