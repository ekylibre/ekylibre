...
namespace :clean do
  ...
  task :models => :environment do
    ...
    models = Object.subclasses_of(ActiveRecord::Base).select{|x| not x.name.match('::')}
    code = "EKYLIBRE_MODELS = ["+models.collect{|m| ":"+m.name.underscore}.sort.join(", ")+"]"
    File.open("#{RAILS_ROOT}/lib/models.rb", "wb") do |f|
      f.write("# Autogenerated from Ekylibre (rake clean:models)\n")
      f.write(code)
    end
  end
