module Diagrams
  ...
  class Graph
    ...
    def arrow(from, to, options = {})
      [:head, :size, :tail].each do |key|
        if options[key]
          options["arrow_#{key}".to_sym] = options.delete(key)
        end
      end
      options[:operator] = "->"
    ...
    def edge(from, to, options = {})
      [:url, :href, :target, :tooltip, :length, :separator].each do |key|
        if options[key]
          options["edge_#{key}".to_sym] = options.delete(key)
        end
      end
    ...
    def write(options = {})
      ...
      File.write(dot_file, self.to_dot)
    ...
    def relational(*models)
      ...
      models.sort{|a,b| a.name <=> b.name }.each do |model|
        ...
        model.reflections.values.each do |reflection|
          next if reflection.macro != :belongs_to or model.name == reflection.class_name or %w(updater creator).include? reflection.name.to_s or (!reflection.polymorphic? and !models.include?(reflection.class_name.constantize))
          arrow_options = {}
  ...
  task relational: :environment do
    {
      ...
      sale: [Sale, SaleNature, SaleItem, OutgoingDelivery, OutgoingDeliveryItem, Transport, IncomingPayment, IncomingPaymentMode, Deposit],
