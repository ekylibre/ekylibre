        ...
        for support in production.supports
          ...
          if area = land_parcel.shape_area
            ...
            Booker.intervene(:plowing, year - 1, 9, 15, 9.78 * coeff, support: support) do |i|
              i.add_cast(reference_name: 'driver',  actor: Booker.find(Worker))
              i.add_cast(reference_name: 'tractor', actor: Booker.find(Product, can: "tow(plower)"))
              i.add_cast(reference_name: 'plow',    actor: Booker.find(Product, can: "plow"))
            ...
            int = Booker.intervene(:sowing, year - 1, 10, 15, 6.92 * coeff, :range => 15, support: support) do |i|
              i.add_cast(reference_name: 'seeds',       actor: Booker.find(Product, variety: :seed, derivative_of: variety))
              i.add_cast(reference_name: 'seeds_to_sow', population: 20)
              i.add_cast(reference_name: 'sower',       actor: Booker.find(Product, can: "sow"))
              i.add_cast(reference_name: 'driver',      actor: Booker.find(Worker))
              i.add_cast(reference_name: 'tractor',     actor: Booker.find(Product, can: "tow(sower)"))
              i.add_cast(reference_name: 'land_parcel', actor: land_parcel)
            ...
            Booker.intervene(:mineral_fertilizing, year, 3, 1, 0.96 * coeff, support: support) do |i|
              i.add_cast(reference_name: 'fertilizer',  actor: Booker.find(Product, variety: :mineral_matter))
              i.add_cast(reference_name: 'fertilizer_to_spread', population: 0.4 + coeff * rand(0.6))
              i.add_cast(reference_name: 'spreader',    actor: Booker.find(Product, can: "spread(mineral_matter)"))
              i.add_cast(reference_name: 'driver',      actor: Booker.find(Worker))
              i.add_cast(reference_name: 'tractor',     actor: Booker.find(Product, can: "tow(spreader)"))
              i.add_cast(reference_name: 'land_parcel', actor: land_parcel)
            end
            ...
            Booker.intervene(:organic_fertilizing, year, 3, 1, 0.96 * coeff, support: support) do |i|
              ...
              i.add_cast(reference_name: 'spreader',    actor: Booker.find(Product, can: "spread(organic_matter)"))
              i.add_cast(reference_name: 'driver',      actor: Booker.find(Worker))
              i.add_cast(reference_name: 'tractor',     actor: Booker.find(Product, can: "tow(spreader)"))
              i.add_cast(reference_name: 'land_parcel', actor: land_parcel)
            end
            ...
            if w.count.modulo(3).zero? # AND NOT prairie
              ...
              Booker.intervene(:chemical_treatment, year, 4, 1, 1.07 * coeff, support: support) do |i|
                i.add_cast(reference_name: 'molecule', actor: Booker.find(Product, can: "kill(plant)"))
                i.add_cast(reference_name: 'molecule_to_spray', population: 0.18 + 0.9 * coeff)
                i.add_cast(reference_name: 'sprayer',  actor: Booker.find(Product, can: "spray"))
                i.add_cast(reference_name: 'driver',   actor: Booker.find(Worker))
                i.add_cast(reference_name: 'tractor',  actor: Booker.find(Product, can: "catch"))
          ...
          if area = land_parcel.shape_area
            ...
            if culture = sowing.casts.find_by(reference_name: 'culture').actor rescue nil
              int = Booker.intervene(:plant_mowing, year, 6, 6, 2.8 * coeff, support: support) do |i|
                ...
                i.add_cast(reference_name: 'tractor',      actor: Booker.find(Product, can: "tow(mower)"))
                i.add_cast(reference_name: 'mower',        actor: Booker.find(Product, can: "mow"))
                i.add_cast(reference_name: 'culture',      actor: culture)
              ...
              Booker.intervene(:straw_bunching, year, 6, 20, 3.13 * coeff, support: support) do |i|
                i.add_cast(reference_name: 'tractor',        actor: Booker.find(Product, can: "tow(baler)"))
                i.add_cast(reference_name: 'baler_driver',   actor: bob)
                i.add_cast(reference_name: 'baler',          actor: Booker.find(Product, can: "bunch"))
      ...
      if Nomen::Varieties[variety].self_and_parents.include?(Nomen::Varieties[:triticum_aestivum]) || Nomen::Varieties[variety].self_and_parents.include?(Nomen::Varieties[:triticum_durum]) || Nomen::Varieties[variety].self_and_parents.include?(Nomen::Varieties[:zea]) || Nomen::Varieties[variety].self_and_parents.include?(Nomen::Varieties[:hordeum])
        ...
        for support in production.supports
          ...
          if area = land_parcel.shape_area
            ...
            bob = Booker.find(Worker)
            other = Booker.find(Worker.where("id != ?", bob.id))
            ...
            if culture = sowing.casts.find_by(reference_name: 'culture').actor rescue nil
              Booker.intervene(:grains_harvest, year, 7, 1, 3.13 * coeff, support: support) do |i|
                i.add_cast(reference_name: 'cropper',        actor: Booker.find(Product, can: "harvest(poaceae)"))
          ...
          if support.storage.is_a?(AnimalGroup)
            for animal in support.storage.members_at()
              Booker.intervene(:animal_treatment, year - 1, 9, 15, 0.5) do |i|
                ...
                i.add_cast(reference_name: 'caregiver',        actor: Booker.find(Worker))
                i.add_cast(reference_name: 'molecule',         actor: Booker.find(AnimalMedicine, can: "care(bos)"))
