  ...
  task :variants => :environment do
    ...
    builder = Nokogiri::XML::Builder.new do |xml|
      xml.send("variants-catalog", locale: I18n.locale, lang: lang, "xml:lang" => lang) do
        Nomen::ProductNatureVariants.list.sort.each do |variant|
          nature = Nomen::ProductNatures.find(variant.nature)
          ...
          attrs["variety-title"] = Nomen::Varieties.find(attrs[:variety]).human_name
          derivative_of = variant.derivative_of || Nomen::ProductNatures.find(variant.nature).derivative_of
          if item = Nomen::Varieties.find(derivative_of)
          ...
          xml.send("variant", attrs) do
            variant.frozen_indicators_values.strip.split(/\s*\,\s*/).each do |couple|
              ...
              indicator = Nomen::Indicators.find(indicator_name)
