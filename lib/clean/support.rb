  ...
  module Support
    ...
    class << self
      ...
      def look_for_labels(*paths)
        ...
        list += Ekylibre::Parts.hash.values.collect{|m| m.keys if m.is_a?(Hash)}.flatten.compact.map(&:to_s)
        list += Ekylibre::Parts.hash.keys.flatten.compact.map(&:to_s)
      ...
      def models_in_file
        ...
        return ObjectSpace
          ...
          .select { |x| not x.name.match(/\AActiveRecord\:\:/) and not x.abstract_class? and not x.name.match(/\AHABTM\_/) }
          .uniq
