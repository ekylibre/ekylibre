...
class InterventionActivitiesTest < ActiveSupport::TestCase
  # test_model_actions
  ...
  test 'destroy intervention update intervention_activities_db_view' do
    ...
    same_intervention = Intervention.where("id IN (SELECT intervention_id FROM activities_interventions WHERE intervention_id = #{first_intervention_id})")

    assert same_intervention.nil?
  end
end
