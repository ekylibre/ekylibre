...
module Isagri
  module Isapaye
    class EcrJournalEntriesExchangerTest < ActiveExchanger::TestCase
      ...
      test 'import' do
        result = Isagri::Isapaye::EcrJournalEntriesExchanger.build(fixture_files_path.join('imports', 'isagri', 'isapaye', 'ecr_journal_entries.ecr'), options: { import_id: @import.id }).run
