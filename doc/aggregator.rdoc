...
These lines describes the major attributes to define the aggregation model:
  ...
  sections(name?, with, level?, if?): # Describe item structure like a section
    title?
    property*
    section*
    sections*
  ...
  matrix(name, with, level?, if?):
    (cell|matrix)+

  title(name, value?, level?, if?):

  property(name, value?, level?, if?):
    
  cell(name, value?, level?, if?):
 
Special markup <within> permits to set default name or value for a group of <title>, <property> or <cell>.

== Sample

These following codes compose the Rosetta Stone to unciffer the Aggregator XML.
  ...
  <aggregators>
    <aggregator name="veterinary_booklet">
      ...
      <sections name="interventions" with="procedures">
        ...
        <matrix name="inputs" with="intervention.variables.of_role(:input)">
...
Then through JasperReports (and a report), it gives: PDF, ODS, ODT, XLSX, DOCX, CSV...
