...
* Install PostGIS (≥= 2.1) from source:
  ...
  * Download and install PostGIS (≥= 2.1) from http://postgis.net:
      ...
      tar xzvf postgis-2.1.1.tar.gz
      cd postgis-2.1.1
      ./configure
      make
      sudo make install
    Or in one line:
      wget http://download.osgeo.org/postgis/source/postgis-2.1.1.tar.gz && tar xzvf postgis-2.1.1.tar.gz && cd postgis-2.1.1 && ./configure && make && sudo make install
...
* Install Ruby 1.9.3 or later. If your distribution isn't up-to-date, see {RVM}[https://rvm.io] or {RbEnv}[https://github.com/sstephenson/rbenv] to install the good version, else:
  ...
  Or compile it from source. In one line:
    wget ftp://ftp.ruby-lang.org/pub/ruby/2.0/ruby-2.0.0-p353.tar.gz && tar xzvf ruby-2.0.0-p353.tar.gz && cd ruby-2.0.0-p353 && ./configure && make && sudo make install
...
* Optionally, it's possible to load anonymized data of a real (french polycultural-cattling) farm :
    rake first_run max=50 folder=<instance>

* Add in /etc/hosts a line to configure a factive subdomain corresponding to your instance:
     127.0.0.1 <instance>.<domain>.<tld>

* Launch server:
    rails server

* Open your web browser and go to http://<instance_name>.<domain>.<tld>:3000
