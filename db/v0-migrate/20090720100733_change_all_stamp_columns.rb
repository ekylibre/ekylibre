class ChangeAllStampColumns < ActiveRecord::Migration
  ...
  def self.up
    for table in EXISTING_TABLES
      ...
      execute "UPDATE #{quoted_table_name(table)} SET creator_id=created_by" if columns.include?(:creator_id) and columns.include?(:created_by)
      execute "UPDATE #{quoted_table_name(table)} SET updater_id=updated_by" if columns.include?(:updater_id) and columns.include?(:updated_by)
  ...
  def self.down
    ...
    for table in EXISTING_TABLES.reverse
      ...
      execute "UPDATE #{quoted_table_name(table)} SET updated_by=updater_id" if columns.include?(:updater_id) and columns.include?(:updated_by)
      execute "UPDATE #{quoted_table_name(table)} SET created_by=creator_id" if columns.include?(:creator_id) and columns.include?(:created_by)
