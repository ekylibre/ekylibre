class AddCompaniesColumns < ActiveRecord::Migration
  def self.up
    ...
    for company in connection.select_all("SELECT companies.id AS cid, pa.id AS pa_id, ca.id AS ca_id FROM #{quoted_table_name(:companies)} AS companies LEFT JOIN #{quoted_table_name(:accounts)} AS pa ON (pa.company_id=companies.id AND pa.number='7') LEFT JOIN #{quoted_table_name(:accounts)} AS ca ON (ca.company_id=companies.id AND ca.number='6')")
      pa_id = company['pa_id']
      ca_id = company['ca_id']
