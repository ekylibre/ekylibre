...
ActiveRecord::Base.transaction do
  ...
  cash = Cash.create!(name: I18n.t('enumerize.cash.nature.cash_box'), nature: "cash_box", :account_id => Account.get("531101", 'enumerize.cash.nature.cash_box'.t).id, :journal_id => journals[:cash].id, country: country)
  baac = Cash.create!(name: I18n.t('enumerize.cash.nature.bank_account'), nature: "bank_account", :account_id => Account.get("51210000", 'enumerize.cash.nature.bank_account'.t).id, :journal_id => journals[:bank].id, :iban => "FR7611111222223333333333391", :mode => "iban", country: :fr)
  ...
  IncomingPaymentMode.create!(name: I18n.t('models.incoming_payment_mode.default.check.name'), cash_id: baac.id, with_accounting: true, :with_deposit => true, :depositables_account => Account.find_or_create_in_chart(:pending_deposit_payments), :depositables_journal_id => journals[:various].id, :attorney_journal_id => journals[:various].id)
