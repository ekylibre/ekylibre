class UpdateParameters < ActiveRecord::Migration
  ...
  def self.down
    ...
    for company in connection.select_all("SELECT * FROM #{quote_table_name(:companies)}")
      for journal in [:sales, :purchases, :bank]
        parameter = select_one("SELECT record_value_id FROM #{quote_table_name(:parameters)} WHERE name LIKE 'accountancy.default_journals.#{journal}'")
        execute "UPDATE #{quote_table_name(:companies)} SET #{journal}_journal_id=#{parameter['record_value_id']}" if parameter
      end
      parameter = select_one("SELECT record_value_id FROM #{quote_table_name(:parameters)} WHERE name LIKE 'management.invoicing.numeration'")
      execute "UPDATE #{quote_table_name(:companies)} SET invoice_sequence_id=#{parameter['record_value_id']}" if parameter
    end

    execute "DELETE FROM #{quote_table_name(:parameters)} WHERE name LIKE 'accountancy.default_journals.%'"
    execute "DELETE FROM #{quote_table_name(:parameters)} WHERE name LIKE 'management.invoicing.numeration'"

    for k, v in CONVERSIONS
      execute "UPDATE #{quote_table_name(:parameters)} SET nature='#{k}' WHERE nature='#{v}'"
    end
    execute "UPDATE #{quote_table_name(:parameters)} SET element_id = record_value_id, element_type = record_value_type"
