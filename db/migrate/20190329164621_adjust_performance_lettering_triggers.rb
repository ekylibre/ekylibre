class AdjustPerformanceLetteringTriggers < ActiveRecord::Migration[4.2]
  def change
    reversible do |dir|
      dir.up do
        ...
        execute <<-SQL.strip_heredoc
          CREATE TRIGGER compute_partial_lettering_status_update
            ...
            FOR EACH ROW
            WHEN ((pg_trigger_depth() = 0) AND (COALESCE(OLD.letter, '') <> COALESCE(NEW.letter, '')
        ...
        execute <<-SQL.strip_heredoc
          DROP TRIGGER IF EXISTS compute_partial_lettering_status ON journal_entry_items
        SQL
