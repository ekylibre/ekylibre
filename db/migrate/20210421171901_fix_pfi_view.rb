class FixPfiView < ActiveRecord::Migration[5.0]
  def up
    execute <<-SQL
      ...
      SELECT
        ...
        ROUND(
          SUM(pip.pfi_value) * ROUND((COALESCE(p.initial_population, 0) /
          COALESCE(ap.size_value, 1)), 2), 2
          ) AS activity_production_pfi_value,
        ROUND(
          ROUND(SUM(pip.pfi_value) * ROUND((COALESCE(p.initial_population, 0) / COALESCE(ap.size_value, 1)), 2), 2) *
          (ap.size_value / (SELECT SUM(aps.size_value) FROM activity_productions aps WHERE aps.activity_id = a.id AND aps.id IN (SELECT activity_production_id FROM activity_productions_campaigns WHERE campaign_id = pip.campaign_id))), 2
          ) AS activity_pfi_value
