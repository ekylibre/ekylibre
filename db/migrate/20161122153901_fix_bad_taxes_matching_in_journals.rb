class FixBadTaxesMatchingInJournals < ActiveRecord::Migration
  def change
    reversible do |d|
      d.up do
        # From sales
        execute 'UPDATE journal_entry_items AS jei SET tax_id = t.id, real_pretax_amount = si.pretax_amount' \
                ...
                ' WHERE je.id = jei.entry_id AND t.collect_account_id = jei.account_id AND -(si.amount - si.pretax_amount) = jei.real_debit'
        # From purchases
