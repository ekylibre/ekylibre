class AddQuantifierForProductionSupports < ActiveRecord::Migration
  ...
  def change
    ...
    execute "UPDATE activities SET family = 'exploitation' WHERE family IS NULL OR family NOT IN (" + FAMILIES.map { |k, _v| "'#{k}'" }.join(', ') + ')'
    families = FAMILIES.select { |_k, v| v[:support_variety] }
    execute 'UPDATE activities SET with_supports = family IN (' + families.map { |k, _v| "'#{k}'" }.join(', ') + '), support_variety = CASE ' + families.map { |k, v| "WHEN family = '#{k}' THEN '#{v[:support_variety]}'" }.join(' ') + ' ELSE NULL END'
    families = FAMILIES.select { |_k, v| v[:cultivation_variety] }
    execute 'UPDATE activities SET with_cultivation = family IN (' + families.map { |k, _v| "'#{k}'" }.join(', ') + '), cultivation_variety = CASE ' + families.map { |k, v| "WHEN family = '#{k}' THEN '#{v[:cultivation_variety]}'" }.join(' ') + ' ELSE NULL END'
