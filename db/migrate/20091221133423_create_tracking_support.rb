class Tracking < ActiveRecord::Migration
  def self.up
    ...
    add_column :product_stocks,  :origin_type, :string   ## productions || purchase_orders || transfers

    add_column :productions, :shape_id, :integer, :references=>:shapes, :on_update=>:cascade, :on_delete=>:cascade
    
    add_column :products, :to_product, :boolean, :null=>false, :default=>false
  ...
  def self.down
    ...
    remove_column :products, :to_product
