  ...
  PRODUCT_NATURE_ITEMS = [
   ...
   PRODUCT_NATURE_VARIANT_READING_ITEMS = [
      ...
      INDICATOR_ITEMS = [
       ...
       {table: 'productions_support_markers', column: 'indicator_name', new: 'sulfur_dioxide_concentration', old: 'sulfur_dioxyde_concentration'},
       ...
       {table: 'productions_support_markers', column: 'indicator_name', new: 'plants_count', old: 'plants_density'},
  ...
  def up
    ...
    for item in PRODUCT_NATURE_VARIANT_READING_ITEMS
      if variant_id = connection.select_value("SELECT min(id) FROM product_nature_variants WHERE reference_name = '#{item[:reference_name]}'").to_i
        if item[:old_frozen_indicator_name] and item[:new_frozen_indicator_name]
        ...
        elsif item[:new_value] and item[:old_value]
