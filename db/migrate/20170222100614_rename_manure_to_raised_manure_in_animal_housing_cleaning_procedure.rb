class RenameManureToRaisedManureInAnimalHousingCleaningProcedure < ActiveRecord::Migration[4.2]
  def change
    reversible do |dir|
      dir.up do
        execute <<-SQL
        UPDATE
          ...
          SET reference_name = 'raised_manure'
          FROM interventions
      ...
      dir.down do
        ...
        UPDATE
          ...
          FROM interventions
          WHERE (iparam.reference_name = 'raised_manure'
            AND interventions.procedure_name = 'animal_housing_cleaning'
            AND iparam.intervention_id = interventions.id)
        SQL
      end
