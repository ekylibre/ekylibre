class CreateMeetingsAndInventories < ActiveRecord::Migration
  def self.up
    ...
    execute "INSERT INTO #{quoted_table_name(:meeting_locations)} (company_id, name, active, created_at, updated_at) SELECT companies.id, 'Divers', #{quoted_true},  current_timestamp, current_timestamp FROM #{quoted_table_name(:companies)} AS companies LEFT JOIN #{quoted_table_name(:meeting_locations)} AS ml ON (ml.company_id=companies.id AND ml.name='Divers') WHERE ml.id IS NULL"

    execute "INSERT INTO #{quoted_table_name(:meeting_modes)} (company_id, name, active,  created_at, updated_at) SELECT companies.id, 'En personne', #{quoted_true},  current_timestamp, current_timestamp FROM #{quoted_table_name(:companies)} AS companies LEFT JOIN #{quoted_table_name(:meeting_modes)} AS mm ON (mm.company_id=companies.id AND mm.name='En personne') WHERE mm.id IS NULL"
