    ...
    reversible do |dir|
      dir.up do
        ...
        WAYS.each do |jn, on, nn|
          execute "UPDATE product_junction_ways SET nature = '#{nn}' FROM product_junctions AS pj WHERE pj.id = junction_id AND pj.nature = '#{jn}' AND product_junction_ways.nature = '#{on}'"
        end
      end
      dir.down do
