class SetParcelItemRoleToNotNilValue < ActiveRecord::Migration[4.2]
  def change
    reversible do |dir|
      dir.up do
        execute set_parcel_items_role_to_service
        execute set_parcel_items_role_to_merchandise
      end
  ...
  def set_parcel_items_role_to_service
    <<-SQL
  ...
  def set_parcel_items_role_to_merchandise
    <<-SQL
	    UPDATE parcel_items pi
  ...
  def set_parcel_items_role_to_null
    <<-SQL
