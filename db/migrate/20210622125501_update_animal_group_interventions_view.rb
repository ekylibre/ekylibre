class UpdateAnimalGroupInterventionsView < ActiveRecord::Migration[5.0]
  def change
    reversible do |dir|
      dir.up do
        execute <<~SQL
          ...
          LEFT JOIN product_memberships as pm ON pm.group_id = animal_group.id
            AND (
          ...
          LEFT JOIN products as animal ON pm.member_id = animal.id AND animal.type = 'Animal'
          ...
          LEFT JOIN product_memberships as pm ON pm.member_id = animal.id
            AND (
            (intervention.started_at BETWEEN pm.started_at AND pm.stopped_at) OR (intervention.started_at > pm.started_at AND pm.stopped_at IS NULL)
            OR (intervention.stopped_at BETWEEN pm.started_at AND pm.stopped_at) OR (intervention.stopped_at > pm.started_at AND pm.stopped_at IS NULL)
            )
          LEFT JOIN products as animal_group ON pm.group_id = animal_group.id AND animal_group.type = 'AnimalGroup'
          GROUP BY intervention.id, animal.id, animal_group.id, pm.group_id;
        SQL
      end
