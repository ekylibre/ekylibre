class MergeEmployeesInUsers < ActiveRecord::Migration
  ...
  def self.up
    ...
    execute "INSERT INTO #{quoted_table_name(:users)} (company_id, created_at, updated_at, first_name, last_name, language_id, name, role_id, office, admin) "+
      "SELECT company_id, created_at, updated_at, first_name, last_name, 0, LOWER("+connection.concatenate("first_name", "'.'", "last_name")+"), 0, id, #{quoted_false} FROM #{quoted_table_name(:employees)} WHERE user_id IS NULL"
    # puts "#{User.all.size} users"
    for user in connection.select_all("SELECT * FROM #{quoted_table_name(:users)} WHERE language_id=0")
...
#      user.save!
      execute "UPDATE #{quoted_table_name(:employees)} SET user_id = #{user['id']} WHERE id = #{user['office']}"
      # puts user.inspect
    end
    ...
    for employee in connection.select_all("SELECT * FROM #{quoted_table_name(:employees)}")
      ...
      # puts hash.inspect
      execute "UPDATE #{quoted_table_name(:users)} SET #{updates.join(', ')} WHERE id=#{employee['user_id']}"
    ...
    remove_index(:employees, :name => "index_#{quoted_table_name(:employees)}_on_company_id")
...
#     remove_index(:employees, :name => "index_employees_on_created_at")
  ...
  def self.down
    ...
    # puts select_one("SELECT count(*) AS x from #{quoted_table_name(:employees)}").inspect

    # Get conversion ids
    employees = {}
    for em in connection.select_all "SELECT user_id, id FROM #{quoted_table_name(:employees)}"
    ...
    execute "UPDATE #{quoted_table_name(:users)} SET deleted = #{quoted_true} WHERE deleted_at IS NOT NULL"
