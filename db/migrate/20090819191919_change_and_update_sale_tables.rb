class ChangeAndUpdateSaleTables < ActiveRecord::Migration
  def self.up
    ...
    execute "UPDATE payment_parts SET downpayment=#{quoted_true} WHERE payment_id IN (SELECT id FROM payments WHERE downpayment)"
  ...
  def self.down
    ...
    execute "UPDATE payments SET downpayment=#{quoted_true} WHERE id IN (SELECT payment_id FROM payment_parts WHERE downpayment)"
