namespace :xil do
  ...
  task :sample => :environment do
    ...
    for angle in 0..1
      ...
      full = true
      if full
        pdf.image("#{RAILS_ROOT}/vendor/plugins/xil/tasks/images/ebimage3.jpg", 300, 20, :width=>275)
        pdf.image("#{RAILS_ROOT}/vendor/plugins/xil/tasks/images/ebimage3.jpg", 300, 300, :height=>100)
        pdf.image("#{RAILS_ROOT}/vendor/plugins/xil/tasks/images/ebimage1.jpg", 420, 300, :height=>100)
        pdf.image("#{RAILS_ROOT}/vendor/plugins/xil/tasks/images/ebimage4.png", 300, 600, :width=>275)
      end
      ...
      if full
        ...
        for j in [50,20*(fs.size+1)*4+50]
          fs.size.times do |i|
            pdf.box(150, i*4*h+j+0*h, 300, 20, 'Hello World!', :font=>{:family=>fs[i], :size=>12})
            pdf.box(150, i*4*h+j+1*h, 300, 20, 'Hello World!', :font=>{:family=>fs[i], :size=>14, :bold=>true})
            pdf.box(150, i*4*h+j+2*h, 300, 20, 'Hello World!', :font=>{:family=>fs[i], :size=>16, :italic=>true})
            pdf.box(150, i*4*h+j+3*h, 300, 20, 'Hello World!', :font=>{:family=>fs[i], :size=>18, :bold=>true, :italic=>true})
          end
        end
      end
    ...
    for i in 0..255
      pdf.box((i/nb)*50+30, (i % nb)*10+30, 10, 20, i.to_s(8).rjust(3,'0'))
      pdf.box((i/nb)*50+55, (i % nb)*10+30, 10, 20, '\\'+i.to_s(8).rjust(3,'0'))
    end
