...
class FPDF
  ...
  def initialize(orientation='P', unit='mm', format='A4')
    ...
    # @fwPt, @fhPt = self.getpageformat(format)
    @DefPageFormat = self.getpageformat(format)
    @CurPageFormat = @DefPageFormat
...
#  end
    if format.is_a? String
      ...
      if format=='a3'
        ...
        self.Error 'Unknown page format: '+format.inspect
      end
    elsif format.is_a? Array
      format[0] *= @k
      format[1] *= @k
    else
      self.Error("Unknown page format:"+foramt.inspect)
    end
  ...
  def objectkey(n)
    return (Md5_16(@encryption_key+([n].pack('VXxx'))))[0..9]
  end
  ...
  def beginpage(orientation, format)
    ...
    # Format
    if format.is_a? String
      format = self.getpageformat(format) 
    elsif not format.is_a? Array
      format = @DefPageFormat
    end
    if orientation!=@CurOrientation or format[0]!=@CurPageFormat[0] or format[1]!=@CurPageFormat[1]
...
#       @CurOrientation=orientation
    ...
    if orientation!=@DefOrientation || format[0]!=@DefPageFormat[0] || format[1]!=@DefPageFormat[1]
  ...
  def GenerateEncryptionKey(user_pass, owner_pass, protection)
    ...
    owner_pass = (owner_pass+@padding)[0..31]
    # Compute O value
  ...
  alias_method :get_width_page      , :GetWidthPage
