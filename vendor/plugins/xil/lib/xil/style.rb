...
module Xil
  ...
  class Style
    ...
    def self.string_to_length(value)
      self.string_to_measure(value, :length, Measure(0, :mm))
    end

    def self.string_to_angle(value)
      self.string_to_measure(value, :angle, Measure(0, :rad))
    end

    def self.string_to_measure(value, dimension, default_value=nil)
      numeric = value[/\d*\.?\d*/].to_f
      raise ArgumentError.new("Unvalid value: #{value.inspect}") if numeric.nil?
      unit = value[/[a-z\%]+/].to_sym
      measure = Measure(numeric.to_f, unit)
      measure = default_value if Measure.dimension(measure.unit) != dimension
      measure
    end
