module Xil
  ...
  class Engine
    ...
    PDF_DEFAULT_UNIT = :pt
    PDF_DEFAULT_FORMAT = Xil::Style::FORMATS[Xil::Style::DEFAULT_FORMAT]
    ...
    def compile_xpdf
      ...
      code += pdf_template(@xml.root, environment)
      return code
    end
    ...
    def pdf_format(element, environment={})
      ...
      @formats[attrs['rule']] = Xil::Style.new(attrs['style'])
      code  = ''
...
#      code += "raise Exception.new('Option :#{name} must be given') if #{name}.nil?\n" unless default
      ...
      code += "||#{default.inspect}" if default
      code += "\n"
      code += "raise Exception.new('Parameter @#{name} must be given') if #{name}.nil?\n" unless default
      code
    end
    ...
    def pdf_page(element, environment={})
      attrs = element.attributes
      style = Xil::Style.new(attrs['style'])
      ...
      environment[:page][:margin] = (style.get('margin')||PDF_DEFAULT_MARGIN).collect{|l| l.convert(PDF_DEFAULT_UNIT).value.round(3)}
      environment[:page][:format] =   (style.get('size')||PDF_DEFAULT_FORMAT).collect{|l| l.convert(PDF_DEFAULT_UNIT).value.round(3)}
      environment[:page][:rotate] = style.get('rotate', '0deg').convert(:deg).to_f
    ...
    def pdf_block(element, environment={})
      ...
      attrs = element.attributes
      style = Xil::Style.new(attrs['style'])
      height = style.get('height', '50mm').convert(PDF_DEFAULT_UNIT).value.round(4)
    ...
    def pdf_set(element, environment={})
      ...
      attrs = element.attributes
      style = Xil::Style.new(attrs['style'])
      left  = style.get('left','0mm').convert(PDF_DEFAULT_UNIT).value.round(4)
      top   = style.get('top','0mm').convert(PDF_DEFAULT_UNIT).value.round(4)
    ...
    def pdf_text(element, environment={})
      ...
      local_style = Xil::Style.new(attrs['style']) 
      style = Xil::Style.new
      style.merge!(@formats[element.name]) if @formats[element.name]
      puts style.inspect
      style.merge!(@formats[".#{attrs['class']}"]) if @formats[".#{attrs['class']}"]
      puts style.inspect
      style.merge!(local_style) if local_style
      puts style.inspect
      
      left   = style.get('left','0mm').convert(PDF_DEFAULT_UNIT).value.round(4)
      top    = style.get('top','0mm').convert(PDF_DEFAULT_UNIT).value.round(4)
      width  = style.get('width','100mm').convert(PDF_DEFAULT_UNIT).value.round(4)
      height = style.get('height','10mm').convert(PDF_DEFAULT_UNIT).value.round(4)
      
      options = {}        
      unless element.text.to_s.blank?
        # options[:text] = string_clean(element.text, environment)
        options[:font] = {}
        options[:font][:size]   = style.get('font-size','10pt').convert(PDF_DEFAULT_UNIT).value
      ...
      if border
        options[:border] = {}
        options[:border][:width] = border[0].convert(PDF_DEFAULT_UNIT).value.round(4)
