  ...
  class SimpleRenderer < Kame::Renderer
    ...
    def footer_code(table)
      ...
      for format, exporter in Kame.exporters
        menu += "<li class=\"export #{exporter.name}\">'+link_to(::I18n.translate('kame.export_as', :format=>::I18n.translate('kame.export.#{format}')).gsub(/\'/,'&#39;'), params.merge(:action=>:#{table.controller_method_name}, :sort=>kame_params[:sort], :dir=>kame_params[:dir], :format=>'#{format}'), :class=>\"icon im-export\")+'</li>"
      end
      menu += "</ul></div>"
