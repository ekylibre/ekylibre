...
module Kame
  ...
  class OpenDocumentSpreadsheetExporter < Kame::Exporter
    ...
    def send_data_code(table)
      ...
      code += "  zile.print('#{self.mime_type}')\n"
      # code += "  zile.print('application/vnd.oasis.opendocument.spreadsheet')\n"
      ...
      code += "  zile.puts('<table:table-column table:number-columns-repeated=\"#{table.exportable_columns.size}\"/>')\n"
      code += "  zile.puts('<table:table-header-rows><table:table-row>"+columns_headers(table).collect{|h| "<table:table-cell table:style-name=\"header\" office:value-type=\"string\"><text:p>'+(#{h}).#{xml_escape}+'</text:p></table:table-cell>"}.join+"</table:table-row></table:table-header-rows>')\n"
