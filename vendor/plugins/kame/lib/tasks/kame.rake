...
namespace :kame do
  ...
  desc "Create public/stylesheets/kame-colors.css"
  task :css do
    ...
    for k0, v0 in dims[0].sort{|a,b| a[0].to_s<=>b[0].to_s}
      ...
      for k1, v1 in dims[1].sort{|a,b| a[0].to_s<=>b[0].to_s}
        ...
        for k2, v2 in dims[2].sort{|a,b| a[0].to_s<=>b[0].to_s}
          ...
          code += "table.kame tr#{dim0}#{dim1} td#{dim2} {background:#{array_to_css(last)}}\n"
          code += "table.kame tr#{dim0}#{dim1}:hover td#{dim2} {background:#{array_to_css(color_merge(last, hover))}}\n"
        end
      end
    end

    File.open(Rails.root.join("public", "stylesheets", "kame-colors.css"), "wb") do |f|
      f.write("/* Auto-generated from plugin Kame (rake kame:css) */\n")
      f.write(code)
    end
  end
